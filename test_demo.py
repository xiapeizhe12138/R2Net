...
def select_model(args, device):
    ...
    if model_id == 0:
        ...
        name, data_range = f"{model_id:00}_RLFN_baseline", 255.0
        ...
        name, data_range = f"{model_id:29}_R2Net", 1.0
        model = R2Net(in_channels=3,out_channels=3,
                    feature_channels=48,upscale=4,bias=False,rep='plain')
        model_path = os.path.join('model_zoo', 'R2Net_for_submit.pth')#os.path.join('model_zoo', 'R2Net_for_submit.pth')
        model.load_state_dict(torch.load(model_path), strict=True)
...
def select_dataset(data_dir, mode):
    ...
    if mode == "test":
        path = [ p for p in sorted(glob.glob(os.path.join(data_dir, "LSDIR_DIV2K_test_LR/*.png"))) ]
...
def run(model, model_name, data_range, tile, logger, device, args, mode="valid"):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_dir", default=r".\results", type=str)
